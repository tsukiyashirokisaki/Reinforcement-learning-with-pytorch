...
class DQNPrioritizedReplay:
    ...
    def learn(self):
        ...
        if self.prioritized:
            ...
            ISWeights = torch.tensor(ISWeights,dtype=torch.float)
        else:
        ...
        if self.prioritized:
            abs_errors = torch.abs(q_eval-q_target).detach().numpy()
            self.memory.batch_update(tree_idx, abs_errors)     # update priority
            loss = torch.sum(ISWeights*(q_eval-q_target)**2)/self.batch_size
        else:
