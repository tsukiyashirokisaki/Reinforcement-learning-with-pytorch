...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        if self.double_q:
            q_eval_next = self.eval_net(torch.tensor(batch_memory[:, -self.n_features:],dtype=torch.float)).max(1)[1].unsqueeze(1)
            q_target = reward + self.gamma*q_next.gather(1,q_eval_next)
        else:
